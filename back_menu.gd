...
func _on_watchadbutton_pressed() -> void:
	...
	print("Player watched ad, applying 3x bonus...")

	var gem_textures := Global.get_collected_gems_textures()
	var score_per_gem := 1000

	# First play the original animation
	for gem_texture in gem_textures:
		if gem_texture == null:
			continue
	...
	# Now play two more animations for the 3x effect
	for i in 2:
		for gem_texture in gem_textures:
			if gem_texture == null:
				continue
	...
	# Apply the 3x bonus
	var bonus_score := base_gem_score_total * 3
	current_session_score += bonus_score
	Global.score += bonus_score
	Global.current_chest_progress = Global.score
	total_multiplied_score = base_gem_score_total * 3
